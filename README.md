...
- **L3′ prior射影**：few-shot が負けた所を「priorの重みを固定し、新しい人を相似変換で*整列だけ*する test-time 射影」で救えるか（few-shot＝重みを曲げる vs 射影＝データを prior へ揃える、の対比）。
...
- **L3′ prior射影（正直な逆結果）**：射影は scratch/few-shot に**届かない**（k=20 で射影 3.66 / few-shot 2.75 / scratch 2.58）。ただし**整列なし prior 5.54 → 射影 3.30** と 7 パラメータで約 **2.2 nats 回復**（天井 1.12）。＝**フレームずれは相似変換で消せるが、多峰な“置き場の関係構造”は本人データでしか埋まらない**。アブレーションでは回復分の大半が平行移動、5例では平行移動のみが最良（自由度過多は過学習）。
